...
HMMDataPrep <- function(metafile_path,
  ...
  # Identify all metadata columns attached to the behavr object
  md <- behavr::meta(dt)

  # Define technical columns that we generally DO NOT need to expand into the dataset
  # (These are used for loading/indexing but rarely for grouping in plots)
  tech_cols <- c("id", "file", "region_id", "start_datetime", "stop_datetime", "duration")

  # Find 'extra' columns (e.g., genotype, replicate, sex, treatment)
  cols_to_expand <- setdiff(names(md), tech_cols)

  if (length(cols_to_expand) > 0) {
    cli::cli_alert_info("Expanding metadata columns: {paste(cols_to_expand, collapse = ', ')}")

    # Efficient update-join: merges columns from metadata (md) into data (dt) by reference
    # This is equivalent to calling xmv() for every column in the list
    dt[md, (cols_to_expand) := mget(paste0("i.", cols_to_expand)), on = "id"]
  }
