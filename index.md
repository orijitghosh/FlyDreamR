...
**FlyDreamR** implements an iterative **hidden Markov model (HMM)**
framework to infer *sleep* and *wake* states from *Drosophila* Activity
Monitor (DAM) **activity counts**. It streamlines the entire analysis
pipeline:

- **Data Loading:** Import raw DAM monitor files and link them with
...
- **Analysis:** Generate publication-ready visualizations and sleep
  metrics.
...
- **Dual analysis modes**: Traditional sleep metrics AND advanced
  HMM-based state inference.
- **Traditional sleep metrics**
  ([`calcTradSleep()`](https://orijitghosh.github.io/FlyDreamR/reference/calcTradSleep.md)):
  Define sleep as you wish, for 5 minutes to 60 minutes of immobility,
  bout counts/lengths, activity index, brief awakenings, day/phase
  summaries.
...
- **Parallel execution** via
  ...
  for big data.
- **Rich visualization suite**: Heatmap hypnograms, faceted profiles,
  and individual profiles, per day.
- **Interactive Shiny GUI** for users who prefer point-and-click
  analysis.
...
To link your raw monitor files with experimental conditions, you must
create a **Metadata CSV file**. This file maps each specific monitor
file and channel to the corresponding fly’s genotype and experiment
timing.
...
Issues and pull requests are welcome! Please visit the [GitHub Issues
page](https://github.com/orijitghosh/FlyDreamR/issues) to report bugs or
request features.
...
For questions or support, please contact the **Author**: Arijit Ghosh,
and **Issues** can be documented at
<https://github.com/orijitghosh/FlyDreamR/issues>.
...
3 (GPL-3)](https://orijitghosh.github.io/FlyDreamR/LICENSE).
